On the top level daffi has only 2 main components. It is `Global` object and `callback` decorator
...
`callback` decorator registers function or class as remote callback. This way function (or all publicly available methods of decorated class) become visible for all nodes by its name.
`callback` decorator can register both synchronous and asynchronous functions/methods
...
In `Process1` we initialize `Global` object to use TCP connection and init controller in this process.
```python
g = Global(process_name=process_name, init_controller=True, host='localhost', port=8888)
```
...
And last step we stop `Global` connection:
```python
g.stop()
```
...
Also `Process2` sleeps forever on this line: 
