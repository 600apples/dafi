...
from daffi.utils.func_validation import func_info
...
class callback(Decorator):
    ...
    def __init__(self, fn: Callable[P, Any]):
        ...
        if isinstance(fn, type):
            self._klass = Callback._init_class(fn)
    ...
    def __call__(self, *args, **kwargs) -> object:
        if self._klass:
            self._klass = Callback._build_class_callback_instance(self._klass, *args, **kwargs)
            return self
...
class fetcher(Decorator):
    """
    fetcher is decorator that converts a decorated method or class into a remote object call.
