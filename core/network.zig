...
pub const Address = union(AddressFamily) {
    ...
    pub fn format(value: @This(), comptime fmt: []const u8, options: std.fmt.FormatOptions, writer: anytype) !void {
        switch (value) {
            inline else => |a| try a.format(fmt, options, writer),
        }
    }
...
//     unix: std.os.sockaddr.un,
// };

pub fn endpointFns(comptime T: type) type {
    return struct {
        pub fn format(self: T, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
            switch (self.address) {
                .ipv4, .ipv6 => try writer.print("{}:{}", .{ self.address, self.port }),
                .unix => try writer.print("{}", .{self.address}),
            }
        }

        pub fn fromSocketAddress(src: *const std.os.sockaddr, size: usize) !T {
            switch (src.family) {
                std.os.AF.INET => {
                ...
                .std.os.AF.UNIX => {
                    const value: *align(4) const std.os.sockaddr.unix = @ptrCast(@alignCast(src));
                    return EndPoint{
                        .address = .{
                            .unix = .{
                                .value = value.path,
        ...
        pub fn toSocketAddress(self: T) SockAddr {
...
pub fn EndPoint(comptime family: AddressFamily) type {
    return comptime switch (family) {
        .ipv4, .ipv6 => struct {
            const Self = @This();
            ...
            pub usingnamespace endpointFns(Self);
        ...
        .unix => struct {
            ...
            pub fn parse(string: []const u8) Self {
                ...
                return Self{ .address = Address{ .unix = .{ .value = buf } } };
            }
        },
    };
}
...
pub const Socket = struct {
    ...
    endpoint: ?EndPoint(.ipv4), // TODO temporary hack
    ...
    pub fn connect(self: *Self, target: EndPoint(.ipv4)) !void {
        ...
        switch (target.toSocketAddress()) {
            inline else => |sockaddr| try connect_fn(self.internal, @ptrCast(&sockaddr), @sizeOf(@TypeOf(sockaddr))),
        }
    ...
    /// for UDP sockets.
    pub fn sendTo(self: Self, receiver: anytype, data: []const u8) SendError!usize {
        const sendto_fn = if (is_windows) windows.sendto else std.os.sendto;
        const flags = if (is_windows or is_bsd) 0 else std.os.linux.MSG.NOSIGNAL;

        return switch (receiver.toSocketAddress()) {
            inline else => |sockaddr| try sendto_fn(self.internal, data, flags, @ptrCast(&sockaddr), @sizeOf(@TypeOf(sockaddr))),
        };
    }
...
pub fn getEndpointList(allocator: std.mem.Allocator, name: []const u8, port: u16) !*EndpointList {
    ...
    if (builtin.link_libc or is_windows) {
        ...
        result.ipv4 = try arena.alloc(EndPoint(.ipv4), addr_count);
        result.ipv6 = try arena.alloc(EndPoint(.ipv6), addr_count);
        ...
        while (it) |info| : (it = info.next) {
            ...
            switch(addr) {
                .ipv4 => result.ipv4[i] = EndPoint(.ipv4){
                    .address = addr,
                    .port = port,
                },
                .ipv6 => result.ipv6[i] = EndPoint(.ipv6){
                    .address = addr,
                    .port = port,
                },
                else => unreachable,
    ...
    if (builtin.os.tag == .linux) {
        ...
        result.ipv4 = try arena.alloc(EndPoint(.ipv4), count);
        result.ipv6 = try arena.alloc(EndPoint(.ipv6), count);
        ...
        for (address_list.addrs) |net_addr| {
            ...
            switch(addr) {
                .ipv4 => result.ipv4[idx] = EndPoint(.ipv4){
                    .address = addr,
                    .port = port,
                },
                .ipv6 => result.ipv6[idx] = EndPoint(.ipv6){
                    .address = addr,
                    .port = port,
                },
                else => unreachable,
...
test "parse Endpoint ipv4" {
    const ep = try EndPoint(.ipv4).parse("123.0.0.1:1234");
    try expect(std.mem.eql(u8, &ep.address.ipv4.value, &[_]u8{ 123, 0, 0, 1 }));
    try expect(ep.port == 1234);
}

test "Connect to an echo server" {

 const endpoint_list = try getEndpointList(std.testing.allocator, "tcpbin.com", 4242);
    defer endpoint_list.deinit();

    for (endpoint_list.ipv4) |endpt| {
     std.debug.print("endpt: {}\n", .{endpt});
    }

    // for (endpoint_list.ipv6) |endpt| {
    //  std.debug.print("endpt: {}\n", .{endpt});
    // }
...
test "UDP timeout" {
    ...
    var sock = try Socket.create(.ipv4, .udp);
    defer sock.close();
    try sock.connect(.{
        .address = .{ .ipv4 = Address.IPv4.init(45, 79, 112, 203) },
        .port = 4242,
    });
